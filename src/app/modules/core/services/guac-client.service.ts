...
import { Client, StringReader, Tunnel } from '@ashishpatel/guacamole-common-js';
import { BehaviorSubject, ReplaySubject, Subject } from 'rxjs';

@Injectable({
  providedIn: 'root'
})
export class GuacClientService {
 ...
 */
public onStateChange = new BehaviorSubject(GuacClientService.STATE.IDLE);
...
public isConnected(): boolean {
    return this.onStateChange.getValue() === GuacClientService.STATE.CONNECTED;
}
...
private handleClientError(status: any): void {
    ...
    this.disconnect();
    this.setState(GuacClientService.STATE.CLIENT_ERROR);
}
...
private handleClientStateChange(state: number): void {
    switch (state) {
        // Idle
        case 0:
            this.setState(GuacClientService.STATE.IDLE);
            break;
        ...
        // Connected + waiting
        case 2:
            this.setState(GuacClientService.STATE.WAITING);
            break;
        // Connected
        case 3:
            this.setState(GuacClientService.STATE.CONNECTED);
            break;
...
private handleTunnelError(status: any): void {
    this.disconnect();
    this.setState(GuacClientService.STATE.TUNNEL_ERROR);
}
...
private handleTunnelStateChange(state: number): void {
    switch (state) {
        ...
        case 1:
            this.setState(GuacClientService.STATE.CONNECTING);
            break;
        // Connection has closed
        case 2:
            this.setState(GuacClientService.STATE.DISCONNECTED);
            break;
    }
}
